...
:: ── 0. Python 32비트 확인 ──
:: [Fix #2] 에러 숨김(2>nul) 제거 → errorlevel 정확히 감지, 창 유지 보장
...
if !errorlevel! neq 0 (
    echo.
    echo  [ERROR] Python 실행에 실패했습니다. 설치 상태를 확인하세요.
    echo.
    pause
    exit /b 1
)
...
if !errorlevel! neq 0 (
    ...
    echo  32비트 Python 설치 경로를 PATH 앞에 추가해 주세요.
    echo.
    pause
    exit /b 1
)
